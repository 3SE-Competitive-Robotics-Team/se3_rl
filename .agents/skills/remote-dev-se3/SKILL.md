...
只验证启动、日志和 checkpoint 链路且必须保留远端现有工作区时，使用
`--sync-mode none --from-scratch`；该模式只编译并运行远端现有代码，不同步源码。
需要短测 checkpoint 保存时再加 `--iterations 5 --save-interval 1`。正式训练不得用
`--sync-mode none` 代替 git 同步。
