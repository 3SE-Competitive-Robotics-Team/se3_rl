...
class _AnalyticVelocityField(torch.nn.Module):
    ...
    def forward(
        self, obs: torch.Tensor, noisy_action: torch.Tensor, t: torch.Tensor
    ) -> torch.Tensor:
        """返回能一步步从噪声积分到目标动作的速度。"""
        del obs
...
def main() -> None:
    """运行合成数据 smoke，确认训练、采样和 checkpoint 都可用。"""
    ...
    if final >= initial:
        raise RuntimeError(f"smoke 失败：loss 未下降 initial={initial:.6f}, final={final:.6f}")

    sampled = sample_actions(model, obs[:8, -1], steps=5)
    if sampled.shape != (8, config.action_dim):
        raise RuntimeError(f"smoke 失败：采样形状错误 {tuple(sampled.shape)}")
    ...
    with TemporaryDirectory() as tmp:
        ...
        if loaded_config != config:
            raise RuntimeError("smoke 失败：checkpoint 配置未正确恢复")
        loaded_sample = sample_actions(loaded, obs[:8, -1], steps=3)
        if loaded_sample.shape != (8, config.action_dim):
            raise RuntimeError(f"smoke 失败：加载后采样形状错误 {tuple(loaded_sample.shape)}")
        if "final_loss" not in metadata:
            raise RuntimeError("smoke 失败：checkpoint metadata 未正确恢复")

    print(f"se3_flow_match smoke passed: initial_loss={initial:.6f}, final_loss={final:.6f}")
...
def _assert_sampler_direction(config: FlowPolicyConfig) -> None:
    ...
    obs = torch.zeros(4, config.obs_dim)
    target = torch.linspace(-0.3, 0.3, config.action_dim).expand(obs.shape[0], -1)
    noise = torch.full_like(target, 0.75)
    model = _AnalyticVelocityField(config, target)
    sampled = sample_actions(model, obs, steps=16, noise=noise)
    if not torch.allclose(sampled, target, atol=1.0e-6):
        raise RuntimeError("smoke 失败：采样积分方向错误")
