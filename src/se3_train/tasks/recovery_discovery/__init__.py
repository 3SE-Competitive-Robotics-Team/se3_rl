...
from .env_cfg import (
    ...
    env_cfg,
    history_env_cfg,
    teacher_history_env_cfg,
    torque_assist_history_env_cfg,
    ungrouped_env_cfg,
)
from .rl_cfg import (
    mlp_rl_cfg,
...
GROUPED_TEACHER_MLP_TASK_ID = "SE3-WheelLegged-Recovery-Loco-Grouped-Teacher-MLP"
GROUPED_GENTLE_TEACHER_MLP_TASK_ID = "SE3-WheelLegged-Recovery-Loco-Grouped-Gentle-Teacher-MLP"
GROUPED_GENTLE_TORQUE_ASSIST_MLP_TASK_ID = (
    "SE3-WheelLegged-Recovery-Loco-Grouped-Gentle-TorqueAssist-MLP"
)
...
def register() -> None:
    ...
    register_mjlab_task(
        task_id=GROUPED_TEACHER_MLP_TASK_ID,
        env_cfg=teacher_history_env_cfg(),
        play_env_cfg=teacher_history_env_cfg(play=True),
        rl_cfg=bind_task_name(teacher_mlp_rl_cfg(), GROUPED_TEACHER_MLP_TASK_ID),
        runner_cls=Se3ProfiledOnPolicyRunner,
    )
    register_mjlab_task(
        task_id=GROUPED_GENTLE_TEACHER_MLP_TASK_ID,
        env_cfg=teacher_history_env_cfg(reward_profile=DiscoveryRewardProfile.GENTLE),
        play_env_cfg=teacher_history_env_cfg(
            play=True,
            reward_profile=DiscoveryRewardProfile.GENTLE,
        ),
        rl_cfg=bind_task_name(
            teacher_mlp_rl_cfg(),
            GROUPED_GENTLE_TEACHER_MLP_TASK_ID,
        ),
        runner_cls=Se3ProfiledOnPolicyRunner,
    )
    register_mjlab_task(
        task_id=GROUPED_GENTLE_TORQUE_ASSIST_MLP_TASK_ID,
        env_cfg=torque_assist_history_env_cfg(),
        play_env_cfg=torque_assist_history_env_cfg(play=True),
        rl_cfg=bind_task_name(
            torque_assist_mlp_rl_cfg(),
            GROUPED_GENTLE_TORQUE_ASSIST_MLP_TASK_ID,
        ),
