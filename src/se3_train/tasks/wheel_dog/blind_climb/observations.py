...
def terrain_height_grid_obs(
    env: ManagerBasedRlEnv,
    sensor_name: str,
    target_height: float,
) -> torch.Tensor:
    """局部地形高度网格，actor 不使用，仅给 critic 做特权观测。"""
    from mjlab.sensor import TerrainHeightSensor

    sensor: TerrainHeightSensor = env.scene[sensor_name]
    heights = sensor.data.heights
    if heights.ndim == 3:
        heights = heights.flatten(start_dim=1)
    centered = float(target_height) - heights
    return torch.clamp(centered, -1.0, 1.0) * 5.0
