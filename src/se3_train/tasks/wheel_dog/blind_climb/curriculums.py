...
def commands_vel(
    ...
    term.cfg.flat_lin_vel_x_range = tuple(
        active.get("flat_lin_vel_x_range", term.cfg.flat_lin_vel_x_range)
    )
    term.cfg.flat_lin_vel_y_range = tuple(
        active.get("flat_lin_vel_y_range", term.cfg.flat_lin_vel_y_range)
    )
    term.cfg.flat_ang_vel_yaw_range = tuple(
        active.get("flat_ang_vel_yaw_range", term.cfg.flat_ang_vel_yaw_range)
    )
    return {
        ...
        "flat_cmd_x_limit": torch.tensor(abs(term.cfg.flat_lin_vel_x_range[1]), device=env.device),
        "flat_cmd_y_limit": torch.tensor(abs(term.cfg.flat_lin_vel_y_range[1]), device=env.device),
...
def terrain_levels(
    ...
    if len(env_ids) == 0:
        return {
            ...
            "progress": torch.tensor(0.0, device=env.device),
            "terrain_level": torch.tensor(0.0, device=env.device),
            "flat_ratio": torch.tensor(0.0, device=env.device),
            "facility_ratio": torch.tensor(0.0, device=env.device),
        }
    if env.common_step_counter == 0:
        facility_mask = terrain_progress.is_facility_terrain(env, env_ids=env_ids)
        return {
            "step_counter": torch.tensor(float(env.common_step_counter), device=env.device),
            "progress": torch.tensor(0.0, device=env.device),
            "terrain_level": terrain.terrain_levels[env_ids].float().mean(),
            "terrain_type": terrain.terrain_types[env_ids].float().mean(),
            "flat_ratio": (~facility_mask).float().mean(),
            "facility_ratio": facility_mask.float().mean(),
    ...
    move_up = facility_mask & (progress_x > current_success) & healthy
    move_down = facility_mask & (progress_x < target_progress) & ~move_up
    num_rows = int(terrain.terrain_origins.shape[0])
    top_sample_min_level = max(0, min(int(top_sample_min_level), num_rows - 1))
    new_levels = terrain.terrain_levels[env_ids].clone()
    facility_levels = new_levels[facility_mask]
    facility_new_levels = (
        facility_levels + move_up[facility_mask].long() - move_down[facility_mask].long()
    )
    facility_new_levels = torch.clamp(facility_new_levels, 0, num_rows - 1)
    top_mask = move_up[facility_mask] & (facility_levels >= num_rows - 1)
    if top_mask.any():
        facility_new_levels[top_mask] = torch.randint(
            top_sample_min_level,
            num_rows,
            (int(top_mask.sum().item()),),
            device=env.device,
            dtype=facility_new_levels.dtype,
        )
    new_levels[facility_mask] = facility_new_levels
    ...
    return {
        ...
        "progress_x": _mean_on_mask(progress_x, facility_mask),
        "success_progress_x": _mean_on_mask(current_success, facility_mask),
        "target_progress_x": _mean_on_mask(target_progress, facility_mask),
        "healthy_ratio": _mean_on_mask(healthy.float(), facility_mask),
        "in_corridor_ratio": _mean_on_mask(in_corridor.float(), facility_mask),
        "lateral_offset_abs": _mean_on_mask(torch.abs(lateral_offset), facility_mask),
        "move_up_ratio": _mean_on_mask(move_up.float(), facility_mask),
        "move_down_ratio": _mean_on_mask(move_down.float(), facility_mask),
