...
_SPEED_END_STEP = 96_000
_TERRAIN_START_STEP = 0
_TERRAIN_END_STEP = 192_000
_PUSH_STAGE_STEPS = (192_000, 256_000)


def env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
    """构造 Stage3 上台阶 GAIT 训练环境。"""
    cfg = base_gait_env_cfg(play=play)
    cfg.scene.terrain = TerrainEntityCfg(
        terrain_type="generator",
        terrain_generator=stage3_terrain_cfg(),
        max_init_terrain_level=3 if play else 0,
    )
    ...
    if not play:
        ...
        cfg.curriculum["terrain_distribution"] = CurriculumTermCfg(
            ...
            params={
                ...
                "start_proportions": (0.45, 0.25, 0.15, 0.15, 0.00),
                "end_proportions": (0.15, 0.15, 0.10, 0.60, 0.00),
                "start_max_level": 0,
                "end_max_level": 3,
            },
        )
...
def stage3_terrain_cfg() -> TerrainGeneratorCfg:
    """Stage3 主训 8-24cm 上台阶，并保留中等复杂地形恢复区。"""
...
__all__ = ["env_cfg", "stage3_terrain_cfg"]
