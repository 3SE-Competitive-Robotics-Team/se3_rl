...
def _mlp_ppo_runner_cfg(smoke: bool = False) -> RslRlOnPolicyRunnerCfg:
    """生成普通 MLP PPO 训练配置。"""
    if smoke or os.environ.get("SE3_SMOKE", "0") == "1":
        max_iterations = 5
        logger = "tensorboard"
...
def single_label_vio_ppo_runner_cfg(
    label: str,
    smoke: bool = False,
) -> RslRlOnPolicyRunnerCfg:
    """FlowMatch 单标签普通 VIO PPO 配置。"""
    cfg = _mlp_ppo_runner_cfg(smoke=smoke)
    cfg.experiment_name = f"se3_wheel_leg_flow_match_{label}_vio"
    cfg.resume = False
    return cfg
