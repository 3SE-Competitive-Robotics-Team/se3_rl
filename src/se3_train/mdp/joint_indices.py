...
def policy_leg_joint_ids(entity: Any) -> tuple[int, ...]:
    """返回 policy 主动杆关节索引。"""
    return joint_ids(entity, JointGroup.POLICY_LEG_NAMES)
...
def leg_actuator_ids(entity: Any) -> tuple[int, ...]:
    ...
    return actuator_ids(entity, JointGroup.POLICY_LEG_NAMES)
...
def joint_ids(
    ...
    return _ids_from_names(entity, "joint_names", names)
...
def actuator_ids(
    ...
    return _ids_from_names(entity, "actuator_names", names)
...
def _ids_from_names(
    ...
    selected = _select_names(available, names)
