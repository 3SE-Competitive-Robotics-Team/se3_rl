...
sim 里没有热模型，策略在 2.5 m/s 指令下能稳定运行（tilt < 1°），但实机轮电机（M3508-Hexroll）在该速度下持续工作时转矩已超出 rated_torque（2.46 N·m），会触发 C620 限流保护或导致电机过热。训练出来的策略不会主动规避高热工况，sim2sim gap 的一部分就来自这里。
