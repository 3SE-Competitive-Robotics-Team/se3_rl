...
2026-09-26 按用户指令在 2155 轮 SIGINT 停止（9 s 退出、显存回 2 MiB），最后 checkpoint `model_2000`（.pt / .onnx）。

- 速度课程第 91 轮推满 2.4（M25–M27 与 `7c29rmkd` 在 214–275 轮），平地跟踪 300 轮到 0.75，与对照同步；
  平地、下台阶、坡道三列的升级节奏不慢于对照。
- 上台阶列等级停在 1.4–1.7（台阶约 5 cm），热身结束后反而下降（900 轮 1.68 → 1481 轮 1.44）；二级台阶门控
  （均级 ≥ 5）不会打开。sim2x 里 model_1400 只在 vx 2.4 时能靠动量冲上复旦场景 20 cm 台阶，低速停在立面前。
- 复旦 7 份快照的奖励函数各段相同，学会上台阶的几段姿态罚是 −10、v1 起才是 −20 → M29 改回 −10，
  见 `docs/plan/m29_fudan_v3_ori10_20260926.md`。
