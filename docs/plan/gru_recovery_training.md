...
所有倾角从第 0 iter 开始进入训练。`init_tilt_bin` 只允许作为日志和验收分桶，不允许进入 reward、command、termination 或 curriculum。
...
- 速度追踪、高度、姿态稳定、接触、默认姿态等项在 reward 函数内部乘 `upright_gate`，倒地时自然降权，站起后自然恢复。
...
| 高度和姿态稳定 | `base_height_l2`、`lin_vel_z_l2`、`ang_vel_xy_l2`、`flat_orientation_l2` | 乘 `upright_gate` 或等价连续 gate；倒地时不应压制起身探索 |
...
2. `Reset/init_tilt_bin_*_ratio` 覆盖所有倾角桶。
3. `Reset/mean_init_tilt_deg` 和 `Reset/max_init_tilt_deg` 符合全角度随机预期。
4. 没有新增 NaN、环境崩溃或异常终止暴涨。

训练中优先看这些 wandb 指标：

| 指标 | 期望方向 | 含义 |
|---|---|---|
| `Reset/init_tilt_bin_*_ratio` | 覆盖全桶 | reset 分布是否真正全角度随机 |
...
- `Reset/init_tilt_bin_*_ratio` 至少出现多个倾角桶。
...
- `Locomotion/upright_gate` 早期可以低，但不应长期完全为 0。
