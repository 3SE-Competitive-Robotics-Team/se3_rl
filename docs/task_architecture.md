...
| `rough/`（临时） | `SE3-WheelLegged-Rough-Exp-StairSpeedCap` / `SE3-WheelLegged-Rough-Exp-HeightWindow` / `SE3-WheelLegged-Rough-Exp-FudanReward` | M26/M27 与 M25 并发对照，各只翻一个开关（见 `docs/plan/m25_m27_pushfix_speedcap_heightwindow_20260925.md`）；M28 整张奖励表换成复旦 v3（见 `docs/plan/m28_fudan_v3_reward_20260926.md`）；对照结束后删除 |
