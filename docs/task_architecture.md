...
| `rough/`（临时） | `SE3-WheelLegged-Rough-Exp-StairSpeedCap` / `SE3-WheelLegged-Rough-Exp-HeightWindow` / `SE3-WheelLegged-Rough-Exp-FudanReward` / `SE3-WheelLegged-Rough-Exp-FudanRewardOri10` / `SE3-WheelLegged-Rough-Exp-FudanRewardOri10Std15` | M26/M27 与 M25 并发对照，各只翻一个开关（见 `docs/plan/m25_m27_pushfix_speedcap_heightwindow_20260925.md`）；M28 整张奖励表换成复旦 v3（见 `docs/plan/m28_fudan_v3_reward_20260926.md`）；M29 在 M28 上把姿态罚改回 −10、高度参考改成复旦格点口径（见 `docs/plan/m29_fudan_v3_ori10_20260926.md`）；M30 在 M29 上把 actor 初始 std 改成 1.5（见 `docs/plan/m30_fudan_v3_ori10_std15_20260926.md`）；对照结束后删除 |
