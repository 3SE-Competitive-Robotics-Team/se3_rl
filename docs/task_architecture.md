...
| `rough/`（临时） | `SE3-WheelLegged-Rough-Exp-StairSpeedCap` / `SE3-WheelLegged-Rough-Exp-HeightWindow` | M26/M27 与 M25 并发对照，各只翻一个开关（见 `docs/plan/m25_m27_pushfix_speedcap_heightwindow_20260925.md`）；对照结束后删除。复旦奖励对照 M28–M33（`...-Exp-FudanReward*` 六个入口与 `fudan_rewards.py`）已于 2026-09-26 删除，结论见 `docs/plan/m28_fudan_v3_reward_20260926.md` 至 `m33_fudan_v3_step5_stair_rewards_20260926.md`，复现用对应 commit |
