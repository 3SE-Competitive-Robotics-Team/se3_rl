...
| `rough/`（临时） | `SE3-WheelLegged-Rough-Exp-StairSpeedCap` / `SE3-WheelLegged-Rough-Exp-HeightWindow` / `SE3-WheelLegged-Rough-Exp-FudanReward` / `SE3-WheelLegged-Rough-Exp-FudanRewardOri10` / `SE3-WheelLegged-Rough-Exp-FudanRewardOri10Std15` / `SE3-WheelLegged-Rough-Exp-FudanRewardOri10Std15NoWarmup` / `SE3-WheelLegged-Rough-Exp-FudanRewardOri10Std15NoWarmupStep5` / `SE3-WheelLegged-Rough-Exp-FudanRewardStep5StairRew` | M26/M27 与 M25 并发对照，各只翻一个开关（见 `docs/plan/m25_m27_pushfix_speedcap_heightwindow_20260925.md`）；M28 整张奖励表换成复旦 v3（见 `docs/plan/m28_fudan_v3_reward_20260926.md`）；M29 在 M28 上把姿态罚改回 −10、高度参考改成复旦格点口径（见 `docs/plan/m29_fudan_v3_ori10_20260926.md`）；M30 在 M29 上把 actor 初始 std 改成 1.5（见 `docs/plan/m30_fudan_v3_ori10_std15_20260926.md`）；M31 在 M30 上去掉平地热身（见 `docs/plan/m31_fudan_v3_ori10_std15_nowarmup_20260926.md`）；M32 在 M31 上把上台阶列最低一级改成 5 cm（见 `docs/plan/m32_fudan_v3_step5_20260926.md`）；M33 在 M32 上加回两项台阶专项奖励（见 `docs/plan/m33_fudan_v3_step5_stair_rewards_20260926.md`）；对照结束后删除 |
